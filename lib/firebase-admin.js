...
function initFirebase() {
  try {
    if (admin.apps.length === 0) {
      ...
      // Converter RSA PRIVATE KEY (PKCS#1) para PRIVATE KEY (PKCS#8) se necessário
      // Resolve erro "DECODER routines::unsupported" no Node 18+/OpenSSL 3
      try {
        const keyObj = crypto.createPrivateKey(privateKey);
        privateKey = keyObj.export({ type: 'pkcs8', format: 'pem' }).toString();
      } catch (e) {
        // Se já for PKCS#8 ou a conversão falhar, usar como está
      }
