...
const PromptsManager: React.FC<PromptsManagerProps> = ({ onClose }) => {
  ...
  const loadPrompts = async () => {
    ...
    try {
      ...
      if (errorMessage.includes('Endpoint /prompts não encontrado') || err?.response?.status === 404) {
        setError('⚠️ Backend não configurado: O endpoint /prompts não existe no backend. Configure o endpoint no n8n para salvar na collection "prompts" do database "messages" no Firestore. Veja README-PROMPTS-FIRESTORE.md para instruções.');
      } else if (errorMessage.includes('Network Error') || errorMessage.includes('Erro de conexão')) {
        setError('⚠️ Erro de conexão: Não foi possível conectar ao backend. Verifique se o endpoint /prompts está configurado no n8n e se está acessível. Os prompts devem ser salvos no Firestore (database "messages", collection "prompts").');
      } else {
  ...
  const handleDelete = async (prompt: Prompt) => {
    ...
    try {
      ...
      const errorMessage = err?.response?.data?.message || 
                          err?.message || 
                          'Não foi possível remover o prompt.';
      setError(errorMessage);
    } finally {
      setSaving(false);
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (errorMessage.includes('Endpoint /prompts não encontrado') || err?.response?.status === 404) {
        setError('⚠️ Backend não configurado: O endpoint POST /prompts não existe. Configure o endpoint no n8n para salvar no Firestore (database "messages", collection "prompts").');
      } else if (errorMessage.includes('Network Error') || errorMessage.includes('Erro de conexão')) {
        setError('⚠️ Erro de conexão: Não foi possível salvar no Firestore. Verifique se o backend está configurado corretamente.');
      } else {
  ...
  return (
    <div className="prompts-manager">
      <div className="prompts-header">
        <div>
          <h3>Prompts da IA</h3>
          <p>Cadastre prompts personalizados para reutilizar nas análises.</p>
        </div>
        {onClose && (
          <button
          ...
          {loading ? (
            ...
            <div className="prompts-empty" style={{ padding: '20px', textAlign: 'center' }}>
              ...
              <div style={{ marginTop: '16px', fontSize: '12px', color: '#9ca3af' }}>
                Consulte o arquivo <strong>README-PROMPTS-FIRESTORE.md</strong> para instruções de configuração.
              </div>
            </div>
          ) : prompts.length === 0 ? (
          ...
          <form className="prompts-form" onSubmit={handleSubmit}>
            <div className="form-group">
              ...
              <input
                ...
                placeholder="Ex: Tom profissional amigável"
                disabled={saving}
              />
            </div>
            <div className="form-group">
              ...
              <input
                ...
                onChange={handleChange}
                placeholder="Ex: Usar em leads de alto valor"
                disabled={saving}
              />
            </div>
            <div className="form-group">
              ...
              <textarea
                ...
                value={form.content}
                onChange={handleChange}
                placeholder="Texto que será usado como instrução para a IA..."
                disabled={saving}
              />
            </div>
